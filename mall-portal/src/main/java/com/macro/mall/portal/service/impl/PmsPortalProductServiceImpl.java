...
public class PmsPortalProductServiceImpl implements PmsPortalProductService {
    ...
    @Override
    public List<PmsProduct> search(String keyword, Long brandId, String productCategoryId, Integer pageNum, Integer pageSize, Integer sort) {
        PageHelper.startPage(pageNum, pageSize);
        PmsProductExample example = new PmsProductExample();
        PmsProductExample.Criteria criteria = example.createCriteria();
        PmsProductExample.Criteria criteria2 = example.or();
        criteria.andDeleteStatusEqualTo(0);
        criteria.andPublishStatusEqualTo(1);
        criteria2.andDeleteStatusEqualTo(0);
        criteria2.andPublishStatusEqualTo(1);
        if (StrUtil.isNotEmpty(keyword)) {
            criteria.andNameLike("%" + keyword + "%");
            criteria2.andSubTitleLike("%" + keyword + "%");
        }
        if (brandId != null) {
        ...
        if (productCategoryId != null) {
            if (productCategoryId.split(",").length > 1) {
                criteria.andProductCategoryIdEqualTo(productCategoryId);
                criteria2.andProductCategoryIdEqualTo(productCategoryId);
            } else {
                criteria.andProductCategoryIdLike("%" + productCategoryId + "%");
                criteria2.andProductCategoryIdLike("%" + productCategoryId + "%");
    ...
    public List<PmsProductCategoryNode> categoryTreeList() {
        ...
        List<PmsProductCategoryNode> result = allList.stream().sorted((a, b) -> {
...
//        result.setBrand(brand);
    ...
    @Override
    public List<PmsProduct> searchRand(Integer verifyStatus, Integer pageNum, Integer pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        PmsProductExample example = new PmsProductExample();
        example.createCriteria().andVerifyStatusEqualTo(verifyStatus)
    ...
    private PmsProductCategoryNode covert(PmsProductCategory item, List<PmsProductCategory> allList) {
        ...
        List<PmsProductCategoryNode> children = allList.stream().sorted((a, b) -> {
            return a.getSort() - b.getSort();

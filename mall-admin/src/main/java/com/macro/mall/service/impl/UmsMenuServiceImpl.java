...
public class UmsMenuServiceImpl implements UmsMenuService {
    ...
    @Override
    public List<UmsMenuNode> treeList(HttpServletRequest request) {
        ...
        List<UmsMenu> collect = menuList.stream().filter(item -> {
            if (item.getId() != menuId && item.getId() != resourceId) {
                return true;
            }
            return false;
        }).collect(Collectors.toList());
        List<UmsMenuNode> result = collect.stream()
                .filter(menu -> menu.getParentId().equals(0L))
